 ...
 * it the only mechanical barrier between an unreviewed main-session edit and the published
 * contract, the build, Storybook, CI and the harness itself. Path matching is the entire security property, so
 ...
 *
 * 1. **The rule as written in `CLAUDE.md` and `.claude/README.md`** — this exact set of paths is
 ...
 *
 * Hermetic: no network, no database, no server. Needs `git` and `node`.
 */
...
describe('the guarded surface (oracle: CLAUDE.md’s list + the single-copy rule)', () => {
  ...
  it('does not restate a single spec/auth path as a literal in the guard code', () => {
    ...
    // first: the header deliberately uses `tokens.css` as a worked example, and prose is not
  ...
  it('has no basename globs today — and records the shape a new one must take', () => {
    // The harness this was ported from guarded `*Dockerfile*`, `*docker-compose*.yml` and
    // `*nginx*.conf`, each leading with `*` because a review found `nginx*.conf` anchored at the
    // start of the basename and a real `widget-test-nginx.conf` therefore unguarded. This package
    // has no container or proxy surface, so those entries were dropped rather than carried over as
    // patterns that can never match. This test is the reminder, and it fails the day someone adds a
    // basename entry without the leading `*` — which is exactly when the lesson applies again.
    const basenameGlobs = GUARDED_PATHS.filter((e) => e.includes('*') && !e.includes('/'))
    expect(basenameGlobs).toEqual([])
    // The one glob that remains is anchored on a directory prefix, and it stays as it is:
    // `settings.json` / `settings.local.json` are the only two names Claude Code reads there.
    expect(GUARDED_PATHS.filter((e) => e.includes('*'))).toEqual(['.claude/settings*.json'])
    ...
    // The exact-match entries are exact on purpose: this is one package today, so a
    // `packages/x/package.json` is NOT guarded. Asserted so it stays a decision.
    expect(guardedMatch('package.json')).toBe('package.json')
    expect(guardedMatch('packages/ds/package.json')).toBeNull()
  })
  ...
  it('resolveLinks follows a dangling link instead of giving up on it', () => {
    ...
    mkdirSync(dir, { recursive: true })
    const target = path.join(dir, 'not-created-yet.css')
...
describe('every guarded entry is refused (oracle: the documented path list)', () => {
...
describe('subagents pass through', () => {
  it('allows a subagent to write the contract doc — that is what the pipeline is for', () => {
    const payload = write({ file_path: 'docs/DESIGN_SYSTEM.md' }, { agent_id: 'agent_01ABC' })
  ...
  it('refuses the same writes without the agent_id — the field is what decides', () => {
    expectRefused(runHook(write({ file_path: 'docs/DESIGN_SYSTEM.md' })), 'docs/DESIGN_SYSTEM.md')
  })
})
...
describe('cwd handling', () => {
  ...
  it('blocks when cwd is outside any git repository, guarded path or not', () => {
    ...
    const guarded = runHook({
      cwd: outside,
      tool_name: 'Write',
      tool_input: { file_path: path.join(repo, 'docs/DESIGN_SYSTEM.md') },
    })
...
describe('symlink routes to a guarded file', () => {
  it('refuses a link that lives OUTSIDE the repo and points into it', () => {
    const target = path.join(repo, 'docs/DESIGN_SYSTEM.md')
    mkdirSync(path.dirname(target), { recursive: true })
    writeFileSync(target, '# contract\n')
    const link = path.join(workspace, 'contract-alias.md')
    ...
    expectRefused(runHook(write({ file_path: link })), 'docs/DESIGN_SYSTEM.md')
  })

  it('refuses a link whose PARENT directory is the symlink', () => {
    const canonicalDir = path.join(repo, 'docs')
    mkdirSync(canonicalDir, { recursive: true })
    const dirLink = path.join(repo, 'contract-dir')
    rmSync(dirLink, { force: true })
    symlinkSync(canonicalDir, dirLink)
    writeFileSync(path.join(dirLink, 'DESIGN_SYSTEM.md'), 'REACHED VIA DIR LINK\n')
    expect(readFileSync(path.join(canonicalDir, 'DESIGN_SYSTEM.md'), 'utf8')).toBe(
      'REACHED VIA DIR LINK\n',
    )
    expectRefused(
      runHook(write({ file_path: path.join(dirLink, 'DESIGN_SYSTEM.md') })),
      'docs/DESIGN_SYSTEM.md',
    )
  })
  ...
  it('does not block a file OUTSIDE the repository that shares a guarded name', () => {
    // The guard is about this repository. Another checkout's `package.json` resolves to
    // `../…/package.json` relative to the root, and a match there would be an over-block that
    // stopped the session touching unrelated files. (It also matters more here than upstream:
    // this harness guards `package.json`, a name almost every directory on disk contains.)
    const outside = path.join(workspace, 'elsewhere')
    mkdirSync(outside, { recursive: true })
    const r = runHook(write({ file_path: path.join(outside, 'package.json') }))
  ...
  it('refuses a `../` route from a cwd that is itself a symlink into the repo', () => {
    ...
    const realDir = path.join(repo, 'scripts')
  ...
  it('refuses a case-variant route, and the route is real on this filesystem', () => {
    ...
    mkdirSync(path.dirname(canonical), { recursive: true })
    writeFileSync(canonical, 'canonical\n')
    const variant = path.join(repo, 'docs/design_system.MD')
    ...
    expectRefused(runHook(write({ file_path: variant })), 'docs/DESIGN_SYSTEM.md')
  })
})
...
describe('the trusted root is the guard’s own location (round 2 — the review FAIL)', () => {
  ...
  //
  // Oracle for every case in this block: the documented rule ("a guarded path is refused however
  ...
  it('refuses an absolute path into THIS repo from a FOREIGN repository’s cwd', () => {
    // Bypass 1 from the upstream review, verbatim: `git init` any directory, hand it in as
    // `cwd`, then name the real guarded file absolutely. Measured exit 0 before the fix.
    const foreign = newRepo()
    const target = path.join(SELF_ROOT, CONTRACT)
  ...
  it('refuses a guarded path in the FOREIGN repo too — a foreign cwd widens, not relocates', () => {
    ...
    const foreign = newRepo()
    expectRefused(
      runHook({ cwd: foreign, tool_name: 'Write', tool_input: { file_path: CONTRACT } }),
      CONTRACT,
    )
  })

  it('refuses an absolute path whose ROOT differs only in case (no cd needed)', () => {
    // Bypass 2 from the upstream review, the more serious one: an absolute path spelled in
    ...
    const canonical = path.join(repo, CONTRACT)
    mkdirSync(path.dirname(canonical), { recursive: true })
    writeFileSync(canonical, 'canonical\n')
    const shouted = path.join(repo.toUpperCase(), CONTRACT)
  ...
  it('refuses the review’s repro: correct cwd, all-lowercase absolute path to the contract', () => {
    ...
    const lowered = path.join(SELF_ROOT.toLowerCase(), CONTRACT)
    expectRefused(
    ...
    if (existsSync(lowered)) {
      expect(readFileSync(lowered, 'utf8')).toBe(readFileSync(path.join(SELF_ROOT, CONTRACT), 'utf8'))
    }
    ...
    expectRefused(
      runHook({
        cwd: newRepo(),
        tool_name: 'Write',
        tool_input: { file_path: path.join(SELF_ROOT.toLowerCase(), CONTRACT) },
      }),
      CONTRACT,
    )
  })
  ...
  it('refuses a root spelled in NFD when the repository is NFC (and the reverse)', () => {
    ...
    const decomposed = path.join(workspace, nfd, CONTRACT)
    expectRefused(
  ...
  it('relativeInside compares SEGMENTS, folded and normalised, and null means outside', () => {
    ...
    expect(relativeInside('/A/B', '/a/b/src/x')).toBe('src/x')
...
describe('the entry point itself cannot be sidestepped (round 2 hardening)', () => {
  it('runs the decision even when the guard is reached through a symlink', () => {
    ...
    const payload = JSON.stringify(write({ file_path: 'docs/DESIGN_SYSTEM.md' }))
  ...
  it('is not pre-empted by NODE_OPTIONS injection', () => {
    ...
    const payload = write({ file_path: 'docs/DESIGN_SYSTEM.md' })
    ...
    expectRefused(runHook(payload, { env: injected }), 'docs/DESIGN_SYSTEM.md')
  })
})
...
describe('malformed input fails closed', () => {
  ...
  it('blocks a tool it cannot read a path out of, rather than waving it through', () => {
    ...
    const r = runHook({
      cwd: repo,
      tool_name: 'Bash',
      tool_input: { command: 'sed -i s/x/y/ docs/DESIGN_SYSTEM.md' },
    })
    expect(r.status).toBe(2)
