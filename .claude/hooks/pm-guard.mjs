 ...
 * place a write to the guarded surface returned exit 0. Hatch and lane are deleted. The guard stays
 ...
 * mechanical barrier between an unreviewed main-session edit and the published-contract surface
 * (styles, tokens, exports, the contract doc and its guardrails) plus the build, Storybook and
 * CI configuration. Hence `pm-guard.test.mjs`, hence the mutation matrix recorded on the
 ...
 * previous hook spawned python3 three times, so this is not a regression).
 ...
 *   .npmrc                   registry and publish configuration; the file a publish token would
 *                            be wired through. Guarded before it exists, on purpose.
 ...
 *   .claude/tools/           the guarded list is imported from `.claude/tools/card-scope.mjs`.
 *                            If the main session can edit that file it can empty the list and
 *                            then edit anything, so leaving it out would make the single-source
 *                            decision into the bypass. A guard whose source of truth is writable
 *                            by the session it guards is not a guard.
 *
 * WHY THIS LIST HAS NO BASENAME GLOBS (unlike the repo it was ported from)
 * The upstream harness guarded `*Dockerfile*`, `*docker-compose*.yml` and `*nginx*.conf`, each
 * deliberately leading with `*` after a review found `nginx*.conf` anchored at the start of the
 * basename and a real `widget-test-nginx.conf` therefore unguarded. This package ships to npm and
 * has no container or proxy surface at all, so those entries were dropped rather than carried over
 * as dead patterns — a guarded path that cannot exist teaches the next reader the wrong thing about
 * what the list is for. **If deploy or container config ever lands here, restore that shape: lead
 * the pattern with `*`.** The one glob that remains is `.claude/settings*.json`, and it is
 * deliberately NOT widened: it is anchored on a directory prefix rather than a basename, and
...
export function guardedMatch(repoRelativePath) {
  ...
  for (const entry of GUARDED_PATHS) {
    ...
    if (e.includes('*')) {
      ...
      // A pattern without a slash is a basename pattern too: a `Dockerfile*`-shaped entry
      // must catch `sub/Dockerfile`, not only a root-level one. No current entry is of that
      // shape (see HOOK_ONLY_PATHS), but the behaviour is kept and tested for when one is.
 ...
 * case, because `ln -s <repo>/src/styles/tokens.css /tmp/x` dangles and
 ...
 *     what catches `src/../src/styles/tokens.css`.
 ...
 *   - lexical alone misses `/tmp/alias -> <repo>/…/tokens.css`;
...
const BLOCK_ADVICE = `
This is the guarded surface — the published contract (the shipped styles and tokens, the export
barrel, the design-system contract document and the guardrails that enforce it), plus the package
manifest, the library build, the Storybook config, CI workflows, and the harness's own settings,
hooks and tools. Print the contract half with:
