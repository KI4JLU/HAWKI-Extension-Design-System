...
# guarded surface: the published contract (`src/styles/`, `styles/`, `src/index.ts`,
# `docs/DESIGN_SYSTEM.md`, `scripts/check-token-usage.sh`, `eslint-plugin/`), plus
# `package.json`, `vite.config.ts`, `.storybook/`, `.github/workflows/`, `.npmrc` and the
# harness's own `.claude/settings*.json`, `.claude/hooks/` and `.claude/tools/`. Those change
# through the pipeline only. Subagent calls (milestone-worker et al.) pass through: their hook
# input carries `agent_id`.
#
...
fail_closed() {
  ...
  printf 'The guarded paths (contract, build, Storybook, CI, harness) cannot be judged, so\n' >&2
