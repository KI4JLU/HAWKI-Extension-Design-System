 ...
 *
 * Hermetic: no network, no database, no server. It needs the `git`
...
describe('the trigger-path list (oracle: CLAUDE.md + docs/DESIGN_SYSTEM.md)', () => {
  ...
  it('--guarded-paths prints the list, with no repository and no git state', () => {
    ...
    //
    // Oracle: CLAUDE.md's mandatory-second-model surface, hand-checked. Asserting the count
    // and two named members is what makes a silently shortened list fail here.
