 ...
 * and points at this array; `.claude/hooks/pm-guard.mjs` imports it as the contract half of
 * what the PM guard refuses. Print it with `--guarded-paths` rather than pasting it into a
 ...
 *   scripts/check-token-usage.sh  the guardrail that enforces the token half of the contract, and
 *   eslint-plugin/                the lint rules card 12 ships to consumers. Both are listed for
 *                                 the same reason `.claude/tools/` is guarded in the hook: a change
